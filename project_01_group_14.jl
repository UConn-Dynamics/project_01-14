...
begin
	...
	const L  = 0.15   # pendulum length [m]
	const m  = 0.1    # bob mass [kg]
	const h1 = 0.2    # frame height [m]
	const w1 = 0.1    # frame width (pivot offset) [m]

	# ─── ODE: state = [θ, θ̇] ────────────────────────────────────────────────────
	# Equation of motion derived from the Lagrangian (see derivation above):
	#   θ̈ = Ω²·cos(θ)·(w1 + L·sin(θ)) / L  -  (g/L)·sin(θ)
	function spinning_pendulum!(du, u, p, t)
		θ, θdot = u
		g, L, Ω, w1 = p
		du[1] = θdot
		du[2] = Ω^2 * cos(θ) * (w1 + L*sin(θ)) / L  -  (g/L)*sin(θ)
	end

	# ============================================================
	# AUTHOR: Mohamed Elgamal
	# Contribution: Added viscous damping extension to the equation
	#               of motion to model air resistance / bearing friction.
	#               Modified EOM:  θ̈ = Ω²cos(θ)(w1+Lsin(θ))/L - (g/L)sin(θ) - b·θ̇
	# ============================================================

	# ─── Damped ODE (extension) ──────────────────────────────────────────────────
	# Adding a viscous damping torque  -b·θ̇  to the equation of motion.
	# This represents air resistance or bearing friction.
	function spinning_pendulum_damped!(du, u, p, t)
		θ, θdot = u
		g, L, Ω, w1, b = p
		du[1] = θdot
		du[2] = Ω^2 * cos(θ) * (w1 + L*sin(θ)) / L  -  (g/L)*sin(θ)  -  b*θdot
	end

	# ─── Initial conditions & time span ─────────────────────────────────────────
	θ0    = 20.0 * (π/180)  # initial angle: 20° from vertical [rad]
	θdot0 = 0.0             # starts from rest [rad/s]
...
md"""# Project\_01 - Spinning Pendulum and the Lagrange Equations

**ME 5180 - Advanced Dynamics | UConn Spring 2026 | Group 14**

| Contributor | Contribution |
|---|---|
| Alireza Hatamirad | 3D animation with bob trajectory trail |
| Mohamed Elgamal | Viscous damping extension |
...
and the pendulum length is $L = 0.15~\text{m}$ with a $m = 0.1~\text{kg}$ point mass at the end. The pendulum swings in the $x'\text{–}z'$ plane as the frame rotates at a constant speed $\Omega$.

![Spinning pendulum diagram](https://raw.githubusercontent.com/cooperrc/me5180-project_01/refs/heads/main/spinning_pendulum.svg)

**Team goals:**
- Build the equations of motion using the Lagrangian $\mathcal{L} = T - V$
- Solve for motion at a slow and a fast rotation speed
- Visualize with plots and a 3D animation
- Extension: add viscous damping and compare

"""

# ╔═╡ a1b2c3d4-0001-11f1-0000-000000000001
md"""## 1 · Lagrangian Derivation

### Position of the bob in the inertial frame

The frame rotates about the vertical ($z$) axis at constant $\Omega$.
The pivot sits at $(w_1,\, 0,\, h_1)$ in the rotating frame $x'\text{–}z'$.
In the rotating frame the bob is at:

$$\mathbf{r}' = \begin{pmatrix} L\sin\theta \\ w_1 \\ h_1 - L\cos\theta \end{pmatrix}$$

Rotating by $\phi = \Omega t$ about $z$ gives the inertial position:

$$x = (w_1 + L\sin\theta)\cos(\Omega t), \quad
  y = (w_1 + L\sin\theta)\sin(\Omega t), \quad
  z = h_1 - L\cos\theta$$

### Velocity and kinetic energy

Differentiating (chain rule, $\dot{\phi}=\Omega$, $\dot{\theta}$ free):

$$\dot{x}^2+\dot{y}^2 = \Omega^2(w_1+L\sin\theta)^2 + L^2\dot{\theta}^2\cos^2\theta + 2\Omega L\dot{\theta}(w_1+L\sin\theta)\cos\theta \cdot (\text{cross terms cancel in }T)$$

After simplification the kinetic energy is:

$$T = \tfrac{1}{2}m\!\left[L^2\dot{\theta}^2 + \Omega^2(w_1 + L\sin\theta)^2\right]$$

### Potential energy

$$V = mgz = mg(h_1 - L\cos\theta)$$

### Lagrangian $\mathcal{L} = T - V$

$$\mathcal{L} = \tfrac{1}{2}m\!\left[L^2\dot{\theta}^2 + \Omega^2(w_1+L\sin\theta)^2\right] - mg(h_1 - L\cos\theta)$$

### Euler–Lagrange equation

$$\frac{d}{dt}\frac{\partial\mathcal{L}}{\partial\dot\theta} - \frac{\partial\mathcal{L}}{\partial\theta} = 0$$

$$mL^2\ddot\theta - m\Omega^2 L\cos\theta(w_1+L\sin\theta) - mgL\sin\theta = 0$$

Dividing by $mL^2$:

$$\boxed{\ddot\theta = \frac{\Omega^2\cos\theta(w_1+L\sin\theta)}{L} - \frac{g}{L}\sin\theta}$$

This is the equation of motion we will integrate numerically.
"""

# ╔═╡ a1b2c3d4-0003-11f1-0000-000000000003
md"""## 2 · Solving the ODE: Slow vs Fast Rotation

We compare two cases:
- **Slow:** $\Omega = 0.5~\text{rad/s}$ — gravity dominates; pendulum swings like a normal pendulum
- **Fast:** $\Omega = 8~\text{rad/s}$ — centrifugal effect dominates; equilibrium shifts outward
"""
...
begin
	# ─── Slow rotation ───────────────────────────────────────────────────────────
	Ω_slow = 0.5
	prob_slow = ODEProblem(spinning_pendulum!, [θ0, θdot0], tspan,
	                       (g, L, Ω_slow, w1))
	sol_slow  = solve(prob_slow, Tsit5(); reltol=1e-9, abstol=1e-9)

	# ─── Fast rotation ───────────────────────────────────────────────────────────
	Ω_fast = 8.0
	prob_fast = ODEProblem(spinning_pendulum!, [θ0, θdot0], tspan,
	                       (g, L, Ω_fast, w1))
	sol_fast  = solve(prob_fast, Tsit5(); reltol=1e-9, abstol=1e-9)

	# ─── Damped fast rotation ─────────────────────────────────────────────────────
	b = 0.3   # damping coefficient [N·m·s/rad]
	prob_damp = ODEProblem(spinning_pendulum_damped!, [θ0, θdot0], tspan,
	                       (g, L, Ω_fast, w1, b))
	sol_damp  = solve(prob_damp, Tsit5(); reltol=1e-9, abstol=1e-9)
...
begin
	...
	p1 = plot(t_plot, [sol_slow(t)[1] * (180/π) for t in t_plot],
	          label="Ω = $(Ω_slow) rad/s (slow)",
	          xlabel="Time [s]", ylabel="θ [degrees]",
	          title="Pendulum angle vs time",
	          lw=2, color=:steelblue)
	plot!(p1, t_plot, [sol_fast(t)[1] * (180/π) for t in t_plot],
	          label="Ω = $(Ω_fast) rad/s (fast)",
	          lw=2, color=:crimson)
	plot!(p1, t_plot, [sol_damp(t)[1] * (180/π) for t in t_plot],
	          label="Ω = $(Ω_fast) rad/s (fast, damped b=$(b))",
	          lw=2, color=:darkorange, linestyle=:dash)

	# ─── θ̇ vs time ───────────────────────────────────────────────────────────────
	p2 = plot(t_plot, [sol_slow(t)[2] for t in t_plot],
	          label="Ω = $(Ω_slow) rad/s (slow)",
	          xlabel="Time [s]", ylabel="θ̇ [rad/s]",
	          title="Angular velocity vs time",
	          lw=2, color=:steelblue)
	plot!(p2, t_plot, [sol_fast(t)[2] for t in t_plot],
	          label="Ω = $(Ω_fast) rad/s (fast)",
	          lw=2, color=:crimson)
	plot!(p2, t_plot, [sol_damp(t)[2] for t in t_plot],
	          label="Ω = $(Ω_fast) rad/s (fast, damped)",
	          lw=2, color=:darkorange, linestyle=:dash)

	# ─── Phase portrait ───────────────────────────────────────────────────────────
	p3 = plot([sol_slow(t)[1]*(180/π) for t in t_plot],
	          [sol_slow(t)[2] for t in t_plot],
	          label="Slow Ω=$(Ω_slow)",
	          xlabel="θ [degrees]", ylabel="θ̇ [rad/s]",
	          title="Phase portrait",
	          lw=2, color=:steelblue)
	plot!(p3, [sol_fast(t)[1]*(180/π) for t in t_plot],
	          [sol_fast(t)[2] for t in t_plot],
	          label="Fast Ω=$(Ω_fast)", lw=2, color=:crimson)
	plot!(p3, [sol_damp(t)[1]*(180/π) for t in t_plot],
	          [sol_damp(t)[2] for t in t_plot],
	          label="Fast damped", lw=2, color=:darkorange, linestyle=:dash)

	plot(p1, p2, p3; layout=(3,1), size=(800, 900))
end

# ╔═╡ a1b2c3d4-0007-11f1-0000-000000000007
md"""## 4 · 3D Animation (Fast Rotation, $\Omega = 8~\text{rad/s}$)

We animate the bob position in the inertial frame.
The rotation matrix about $z$ is:

$$R_z(\phi) = \begin{pmatrix}\cos\phi & -\sin\phi & 0\\\sin\phi & \cos\phi & 0\\0 & 0 & 1\end{pmatrix}$$

The pivot and bob positions (inertial) are:

$$\mathbf{p}_\text{pivot}(t) = R_z(\Omega t)\begin{pmatrix}0\\w_1\\h_1\end{pmatrix}, \quad
  \mathbf{r}_\text{bob}(t) = R_z(\Omega t)\begin{pmatrix}L\sin\theta\\w_1\\h_1-L\cos\theta\end{pmatrix}$$
"""

# ╔═╡ a1b2c3d4-0008-11f1-0000-000000000008
begin
	# ============================================================
	# AUTHOR: Alireza Hatamirad
	# Contribution: 3D animation of the spinning pendulum with
	#               bob trajectory trail in the inertial frame.
	# ============================================================

	# ─── Rotation matrix about z-axis ────────────────────────────────────────────
	Rz(φ) = [cos(φ) -sin(φ) 0.0;
	          sin(φ)  cos(φ) 0.0;
	          0.0     0.0    1.0]

	# ─── Inertial positions ───────────────────────────────────────────────────────
	pivot_pos(t; Ω=Ω_fast) = Rz(Ω*t) * [0.0, w1, h1]
	bob_pos(θ, t; Ω=Ω_fast) = Rz(Ω*t) * [L*sin(θ), w1, h1 - L*cos(θ)]

	# ─── Sample trajectory at 60 fps ─────────────────────────────────────────────
	fps = 60
	ts  = range(tspan[1], tspan[2]; length = Int(round((tspan[2]-tspan[1])*fps)) + 1)
	θs  = [sol_fast(ti)[1] for ti in ts]

	xs = Float64[]; ys = Float64[]; zs = Float64[]
	px = Float64[]; py = Float64[]; pz = Float64[]
	for (ti, θi) in zip(ts, θs)
		b_vec = bob_pos(θi, ti)
		p_vec = pivot_pos(ti)
		push!(xs, b_vec[1]); push!(ys, b_vec[2]); push!(zs, b_vec[3])
		push!(px, p_vec[1]); push!(py, p_vec[2]); push!(pz, p_vec[3])
	end

	# ─── Axis limits ─────────────────────────────────────────────────────────────
	pad = 0.05
	xmin, xmax = minimum([xs; px; 0.0])-pad, maximum([xs; px; 0.0])+pad
	ymin, ymax = minimum([ys; py; 0.0])-pad, maximum([ys; py; 0.0])+pad
	zmin, zmax = minimum([zs; pz; 0.0])-pad, maximum([zs; pz; 0.0])+pad

	# ─── Build GIF ────────────────────────────────────────────────────────────────
	origin = [0.0, 0.0, 0.0]
	top    = [0.0, 0.0, h1]
	trail_len = Int(round(1.0*fps))   # 1-second trail
	...
	anim = @animate for k in eachindex(ts)
		...
		xt = xs[k0:k]; yt = ys[k0:k]; zt = zs[k0:k]

		plt = plot(
			xlim=(xmin,xmax), ylim=(ymin,ymax), zlim=(zmin,zmax),
			xlabel="x (m)", ylabel="y (m)", zlabel="z (m)",
			legend=false,
			title="Spinning Pendulum 3D   Ω=$(Ω_fast) rad/s   t=$(round(ti,digits=2)) s",
			camera=(35, 25)
		)
		# Vertical pole: origin → top
		plot!(plt, [origin[1],top[1]], [origin[2],top[2]], [origin[3],top[3]], lw=4, color=:gray)
		# Horizontal arm: top → pivot
		plot!(plt, [top[1],pv[1]], [top[2],pv[2]], [top[3],pv[3]], lw=4, color=:blue)
		# Pendulum rod: pivot → bob
		plot!(plt, [pv[1],bv[1]], [pv[2],bv[2]], [pv[3],bv[3]], lw=4, color=:red)
		# Bob (mass)
		scatter!(plt, [bv[1]], [bv[2]], [bv[3]], markersize=8, color=:red)
		# Trail
		plot!(plt, xt, yt, zt, lw=2, color=:orange, alpha=0.6)
		plt
	end
	gif(anim, "spinning_pendulum_3D.gif", fps=60)
end

# ╔═╡ a1b2c3d4-0009-11f1-0000-000000000009
md"""## 5 · Discussion

**Slow rotation ($\Omega = 0.5~\text{rad/s}$)**  
The centrifugal term $\Omega^2\cos\theta(w_1+L\sin\theta)/L$ is small compared to $g/L \approx 65~\text{s}^{-2}$.
The pendulum behaves almost like a simple gravity pendulum — periodic oscillation near $\theta = 0$.

**Fast rotation ($\Omega = 8~\text{rad/s}$)**  
The centrifugal term becomes significant. The equilibrium shifts outward to a larger angle
where centrifugal and gravitational torques balance. The bob oscillates around this new equilibrium.
The critical spin-up speed (where $\theta = 0$ becomes unstable) is $\Omega_c = \sqrt{g/L} \approx 8.1~\text{rad/s}$.

**Damping extension**  
Adding a viscous damper $-b\dot\theta$ causes the oscillation to decay toward the equilibrium angle.
This is more physically realistic (bearing friction, air drag).
The modified equation of motion is:

$$\ddot\theta = \frac{\Omega^2\cos\theta(w_1+L\sin\theta)}{L} - \frac{g}{L}\sin\theta - b\dot\theta$$
"""
...
manifest_format = "2.0"
project_hash = "5175158c1ea62fa7a655bb44ddbef2681c1385da"
...
git-tree-sha1 = "0609b130193a90bb233ebd2027bc7622d0d4ef25"
...
git-tree-sha1 = "2346652a682b20a92c7b1c541bdc2eba316ef070"
